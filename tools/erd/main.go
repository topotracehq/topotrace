/*******************************************************************************
 * @file         main.go
 * @brief        Command erd generates TopoTrace's data-model documentation from the Go types themselves: docs/data-model.md (entity tables plus the relationships between them) and docs/data-model.svg (the entity relationship diagram the README shows).
 * @project      TopoTrace
 *
...
// Command erd generates TopoTrace's data-model documentation from the Go
...
// Go has no foreign keys, and TopoTrace deliberately does not carry ORM
...
func renderMarkdown(entities []entity) []byte {
	...
	b.WriteString(`TopoTrace's persisted shapes, generated from ` + "`internal/model/types.go`" + ` so
