...
Deploys the whole stack on Kubernetes: the muster server (Deployment +
...
- The images built locally (nothing is published to a registry):
  ...
  ```
  kind load docker-image muster:latest
  kind load docker-image muster-ubuntu-agent:latest
  ```
  With `minikube`:
  ```
  minikube image load muster:latest
  minikube image load muster-ubuntu-agent:latest
  ```
...
Read what it printed (`helm status muster` to see it again) -- it has the
...
changes three templates at once: it adds `MUSTER_AUTH_TOKEN` to the
server Deployment, and `--token`/`MUSTER_TOKEN` to whichever agent
...
the cluster: only pods labeled `muster.io/network-role: agent` (this
