...
<#
.SYNOPSIS
    Minimal Windows agent for TopoTrace: collects basic system facts and
    pushes them to a TopoTrace ingest daemon over the TOPOTRACE1 wire protocol.
...
.DESCRIPTION
    ...
    show up in TopoTrace, the same way `go run ./cmd/demoagent` does for
    Linux test fixtures.
    ...
    same TOPOTRACE1 header-plus-payload protocol cmd/demoagent uses:
    ...
    Invoke-TopoTraceAction below, then reports the outcome back over a
    second, short TOPOTRACE1-RESULT connection. See
...
.PARAMETER TopoTraceHost
    Hostname or IP of the machine running the TopoTrace ingest daemon.
...
.PARAMETER HostName
    Name to report this host as in TopoTrace. Defaults to the local computer
...
.EXAMPLE
    .\topotrace-agent.ps1 -TopoTraceHost 192.168.1.50 -TopoTracePort 9090

    Scans this machine and sends it to the ingest daemon at
    192.168.1.50:9090, reporting as this machine's computer name.

.EXAMPLE
    .\topotrace-agent.ps1 -TopoTraceHost localhost -HostName winbox01 -KeepFiles
...
.EXAMPLE
    .\topotrace-agent.ps1 -TopoTraceHost 192.168.1.50 -Token $env:TOPOTRACE_TOKEN
...
.NOTES
    ...
    capture-file format TopoTrace's Go server expects, and that server-side
    ...
    script: try it against a test/dev TopoTrace instance first.
#>
[CmdletBinding()]
param(
    ...
    [string]$OutDir = (Join-Path $env:TEMP "topotrace-agent-$([guid]::NewGuid().ToString('N').Substring(0,8))"),
    ...
    # stdout) -- for a host with no route to TopoTrace at all. See
...
function Assert-SafeToken {
    ...
    if ($Value.Length -eq 0 -or $Value.Length -gt 128 -or ($Value -notmatch '^[a-zA-Z0-9._-]+$')) {
        throw "$Name '$Value' is not a valid TopoTrace token -- must be 1-128 chars of letters, digits, '.', '_', '-' only (this matches the server's own validation, so a bad token would be rejected there anyway)."
    }
}
...
if (-not $TopoTraceHost -and -not $AirgapOut) {
    throw "-TopoTraceHost is required (or -AirgapOut, for a host with no network route to TopoTrace at all -- see agent/airgap/README.md)."
}

# Send-TopoTraceActionResult opens a short second TCP connection to report
...
# report).
function Send-TopoTraceActionResult {
    param(
        [string]$TopoTraceHost, [int]$TopoTracePort, [string]$Token,
        [string]$ActionId, [string]$Status, [string]$Detail
    )
    $detailBytes = [System.Text.Encoding]::UTF8.GetBytes($Detail)
    $client = [System.Net.Sockets.TcpClient]::new()
    try {
        $client.Connect($TopoTraceHost, $TopoTracePort)
        $stream = $client.GetStream()
        $header = "TOPOTRACE1-RESULT $Token $ActionId $Status $($detailBytes.Length)`n"
...
# Invoke-TopoTraceAction executes an allow-listed remediation action the
...
function Invoke-TopoTraceAction {
    param(
        [Parameter(Mandatory = $true)][string]$Line,
        [Parameter(Mandatory = $true)][string]$TopoTraceHost,
        [Parameter(Mandatory = $true)][int]$TopoTracePort,
    ...
    Send-TopoTraceActionResult -TopoTraceHost $TopoTraceHost -TopoTracePort $TopoTracePort -Token $Token -ActionId $actionId -Status $status -Detail $detail
}
...
function Export-TopoTraceExtra {
...
Export-TopoTraceExtra -Name "disks.txt" -Collect {
    Get-CimInstance -ClassName Win32_LogicalDisk -Filter "DriveType=3" |
        Select-Object DeviceID, Size, FreeSpace
}

Export-TopoTraceExtra -Name "software.txt" -Collect {
    Get-ItemProperty @(
...
Export-TopoTraceExtra -Name "services_running.txt" -Collect {
    Get-Service | Where-Object { $_.Status -eq "Running" } |
        Select-Object Name, DisplayName, Status
}

Export-TopoTraceExtra -Name "ports.txt" -Collect {
    Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue |
        Select-Object LocalAddress, LocalPort
}

Export-TopoTraceExtra -Name "users.txt" -Collect {
    Get-LocalUser | Select-Object Name, Enabled
}

Export-TopoTraceExtra -Name "netadapters.txt" -Collect {
    Get-NetIPAddress -ErrorAction SilentlyContinue |
        Select-Object InterfaceAlias, AddressFamily, IPAddress
}

Export-TopoTraceExtra -Name "scheduledtasks.txt" -Collect {
...
Export-TopoTraceExtra -Name "hotfixes.txt" -Collect {
...
Export-TopoTraceExtra -Name "certs.txt" -Collect {
...
Export-TopoTraceExtra -Name "firewall.txt" -Collect {
...
# agent/ubuntu/topotrace-agent.sh): browser, user/profile, id, version,
...
# --- air-gapped output (no network path to TopoTrace at all) ----------------
...
if ($AirgapOut) {
    ...
    if ($AirgapOut -eq "-") {
        ...
        Write-Host "Move this file to any machine that can reach TopoTrace and POST it, e.g.:"
        Write-Host "  curl -sS -X POST 'http://<topotrace-host>:8080/api/airgap-report' -H 'Content-Type: application/json' --data '@$AirgapOut'"
    }
    if (-not $KeepFiles) {
...
# --- send over TOPOTRACE1 ----------------------------------------------------
Write-Host "Connecting to ${TopoTraceHost}:${TopoTracePort} ..."
$client = [System.Net.Sockets.TcpClient]::new()
$actionLine = $null
try {
    $client.Connect($TopoTraceHost, $TopoTracePort)
    ...
    $header = "TOPOTRACE1 $Platform $HostName $AuthToken $($payload.Length)`n"
    $headerBytes = [System.Text.Encoding]::ASCII.GetBytes($header)
    $stream.Write($headerBytes, 0, $headerBytes.Length)
    ...
    if ($reply -notmatch '^OK\b') {
        throw "TopoTrace ingest daemon did not report success: $reply"
    }
}
finally {
    $client.Close()
...
if ($actionLine -and $actionLine.StartsWith("ACTION")) {
    Invoke-TopoTraceAction -Line $actionLine -TopoTraceHost $TopoTraceHost -TopoTracePort $TopoTracePort -Token $AuthToken
}
