...
/**
 * Posts a device report to TopoTrace's `POST /api/mobile-report` endpoint.
 *
 * Deliberately written against plain java.net.HttpURLConnection rather
 * than OkHttp/Retrofit: TopoTrace's philosophy throughout (see every other
 * agent script, and internal/ingest's hand-rolled TOPOTRACE1 protocol) is
 ...
 * response body) surfaces as a [TopoTraceReportException] with a message
 ...
 */
class TopoTraceReportException(message: String, cause: Throwable? = null) : Exception(message, cause)

data class TopoTraceReportResult(val statusCode: Int, val body: String)

object TopoTraceClient {
     ...
     * minted for this host in the TopoTrace web UI's Agents tab).
     *
     * [baseUrl] should be a bare origin like "https://topotrace.example.com"
     * or "http://192.168.1.20:8080" -- no trailing slash required, this
     * function normalizes it.
     */
    @Throws(TopoTraceReportException::class)
    fun postMobileReport(
    ...
    ): TopoTraceReportResult {
        val trimmedBase = baseUrl.trim().trimEnd('/')
        if (trimmedBase.isEmpty()) {
            throw TopoTraceReportException("Server URL is empty")
        }
        val url = try {
            URL("$trimmedBase/api/mobile-report")
        } catch (e: Exception) {
            throw TopoTraceReportException("Invalid server URL: ${e.message}", e)
        }

        val bodyJson = TopoTraceJson.encode(
            mapOf(
                "host" to host,
        ...
        val conn = try {
            ...
            throw TopoTraceReportException("Couldn't open connection: ${e.message}", e)
        }
        ...
        try {
            ...
            if (status !in 200..299) {
                throw TopoTraceReportException("Server returned HTTP $status: ${responseBody.take(300)}")
            }
            return TopoTraceReportResult(status, responseBody)
        } catch (e: TopoTraceReportException) {
            throw e
        } catch (e: IOException) {
            throw TopoTraceReportException("Network error talking to TopoTrace: ${e.message}", e)
        } finally {
            conn.disconnect()
        }
    }
