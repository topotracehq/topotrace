#!/usr/bin/env bash
################################################################################
# @file         topotrace-agent.sh
# @brief        Minimal Ubuntu/Linux agent for TopoTrace: collects basic system facts and pushes them to a TopoTrace ingest daemon over the TOPOTRACE1 wire protocol.
# @project      TopoTrace
#
...
#
# Minimal Ubuntu/Linux agent for TopoTrace: collects basic system facts and
# pushes them to a TopoTrace ingest daemon over the TOPOTRACE1 wire protocol.
#
# This is the Linux counterpart to agent/windows/topotrace-agent.ps1 (and,
...
# same TOPOTRACE1 header-plus-payload protocol cmd/demoagent and the Windows
# agent both use:
#
...
# TOPOTRACE1-RESULT connection. See internal/remediate/actions.go for the
...
#
# The capture file names/contents are TopoTrace's own raw-capture contract
...
#
# Usage:
#   ./topotrace-agent.sh --topotrace-host <host> [options]
#
# Options:
#   --topotrace-host HOST   Required. Hostname/IP of the TopoTrace ingest daemon.
#   --topotrace-port PORT   Ingest daemon TCP port (default: 9090).
#   --host-name NAME     Name to report this host as in TopoTrace
...
#
# Examples:
#   ./topotrace-agent.sh --topotrace-host 192.168.1.50
#   ./topotrace-agent.sh --topotrace-host localhost --host-name webbox01 --keep-files
#   ./topotrace-agent.sh --topotrace-host 192.168.1.50 --token "$TOPOTRACE_TOKEN"
#
...
# test/dev TopoTrace instance before trusting it on anything that matters.
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --topotrace-host) TOPOTRACE_HOST="$2"; shift 2 ;;
        --topotrace-port) TOPOTRACE_PORT="$2"; shift 2 ;;
...
if [[ -z "$TOPOTRACE_HOST" && -z "$AIRGAP_OUT" ]]; then
    echo "Error: --topotrace-host is required (or --airgap-out, for a host with no network route to TopoTrace at all -- see agent/airgap/README.md)." >&2
    exit 1
fi
...
assert_safe_token() {
    ...
    if [[ ${#value} -eq 0 || ${#value} -gt 128 || ! "$value" =~ ^[a-zA-Z0-9._-]+$ ]]; then
        echo "Error: $name '$value' is not a valid TopoTrace token -- must be 1-128 chars of letters, digits, '.', '_', '-' only (this matches the server's own validation, so a bad token would be rejected there anyway)." >&2
        exit 1
    fi
}
...
CLEANUP_DIR=""
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="$(mktemp -d -t topotrace-agent.XXXXXX)"
    CLEANUP_DIR="$OUT_DIR"
else
    mkdir -p "$OUT_DIR"
fi
...
# report_action_result opens a short second TOPOTRACE1-RESULT connection to
...
run_action() {
    ...
    logfile="$(mktemp -t topotrace-action.XXXXXX)"
    case "$verb" in
        restart-service)
...
# reach TopoTrace, and POST it from there.
if [[ -n "$AIRGAP_OUT" ]]; then
    ...
    if [[ "$AIRGAP_OUT" == "-" ]]; then
        ...
        echo "Move this file to any machine that can reach TopoTrace and POST it, e.g.:"
        echo "  curl -sS -X POST 'http://<topotrace-host>:8080/api/airgap-report' -H 'Content-Type: application/json' --data @$AIRGAP_OUT"
    fi
...
echo "Connecting to ${TOPOTRACE_HOST}:${TOPOTRACE_PORT} ..."
exec 3<>"/dev/tcp/${TOPOTRACE_HOST}/${TOPOTRACE_PORT}"

printf 'TOPOTRACE1 %s %s %s %s\n' "$PLATFORM" "$HOST_NAME" "$AUTH_TOKEN" "$payload_size" >&3
cat "$archive" >&3
...
if [[ "$reply" != OK* ]]; then
    echo "Error: TopoTrace ingest daemon did not report success: $reply" >&2
    exit 1
fi
