...
# @brief        Minimal macOS agent for TopoTrace: collects basic system facts and pushes them to a TopoTrace ingest daemon over the TOPOTRACE1 wire protocol.
# @project      TopoTrace
#
...
#
# Minimal macOS agent for TopoTrace: collects basic system facts and pushes
# them to a TopoTrace ingest daemon over the TOPOTRACE1 wire protocol.
#
...
# agent/ubuntu/topotrace-agent.sh.
#
...
if [[ -z "$TOPOTRACE_HOST" ]]; then
    echo "Error: --topotrace-host is required." >&2
    exit 1
fi
...
CLEANUP_DIR=""
if [[ -z "$OUT_DIR" ]]; then
    OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/topotrace-agent.XXXXXX")"
    CLEANUP_DIR="$OUT_DIR"
else
    mkdir -p "$OUT_DIR"
fi
...
# --- browser extensions ------------------------------------------------
# Same line format as the Linux agent (see agent/ubuntu/topotrace-agent.sh):
