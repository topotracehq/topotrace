...
```
go run ./cmd/topotrace -ai-api-key "sk-ant-..."
# or: TOPOTRACE_AI_API_KEY=sk-ant-... go run ./cmd/topotrace
```
...
4. **Log it.** The question and answer (both truncated) are recorded
   to the audit log as an `ask-topotrace` entry, actor-attributed the same
